...
def get_user_id_by_username(username: str) -> str:
    ...
    try:
        ...
        query = "SELECT user_id FROM house_owner WHERE user_name = %s"
...
def get_username_by_email(email: str) -> str:
    ...
    try:
        ...
        query = "SELECT user_name FROM house_owner WHERE mail_id = %s"
...
async def signup(user: UserSignUp):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM house_owner WHERE user_name = %s", (user.name,))
        if cursor.fetchone():
            raise HTTPException(status_code=400, detail="Username already exists")

        # Check if the email already exists
        cursor.execute("SELECT * FROM house_owner WHERE mail_id = %s", (user.email,))
        if cursor.fetchone():
            raise HTTPException(status_code=400, detail="Email already exists")

        # Insert the new user
        insert_query = "INSERT INTO house_owner (user_name, mail_id, password_) VALUES (%s, %s, %s)"
...
async def login(user: UserLogin):
    ...
    try:
        ...
        select_query = "SELECT * FROM house_owner WHERE mail_id = %s"
